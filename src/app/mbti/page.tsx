...
// ✅ members를 컴포넌트 밖으로 빼서 "렌더마다 재생성" 방지
const MEMBERS = [
...
export default function Home() {
  ...
  // ✅ props 안정화 (캔버스 메모/리렌더에 유리)
  const handleCenterChange = useCallback((id: string) => {
    setCenterId(id);
  }, []);

  // ✅ (선택) 조합 캐시: center 바뀌어도 같은 mbti 조합은 재사용 가능
  // 지금은 데모라 체감 적지만, 추후 계산이 무거워지면 도움이 됨
        ...
        <section className="mt-4">
          <div className="rounded-3xl bg-white/70 p-6 shadow-sm ring-1 ring-black/5">
            <h1 className="text-3xl font-extrabold leading-tight">
              우리 모임<br />
              <span className="underline decoration-[#FDD835]/70">누가 제일 잘 맞을까?</span>
            </h1>
